      ...
      <template v-if="tab == 'query'">
        ...
        <div class="q-px-sm">
          ...
          <q-markup-table class="q-pb-md table-no-padding table-bordered" flat dense wrap-cells>
            ...
            <tbody>
              <s-drag v-model="dataModel.config.query.filters" container="tr">
                <template #default="{ item: v, index: i }">
                  <td class="text-center"><q-btn flat dense color="negative" icon="delete" @click="delQueryFilter(v, i)" /></td>
                  <td-input :i="i" v-model="v.name" type="select" :options="rawData.cols" />
                  <td-input :i="i" v-model="v.operator" type="select" :options="getOptOperator(v.name)" />
                  <td-input v-if="v.operator == 'order'" :i="i" v-model="v.value" type="select" :options="['ASC', 'DESC']" />
                  <td-input v-else :i="i" v-model="v.value" :type="getColType(v.name)" />
                </template>
              </s-drag>
            </tbody>
          </q-markup-table>
...
import type { WidgetData } from '~/utils/gridstack'
...
const getOptOperator = (name: string) => {
