<template>
  <div>
        ...
        <div class="col-12">
          <q-card flat style="border-radius: 8px 0px 0px 0px" class="q-pa-sm">
            <div class="flex justify-end items-center q-gutter-sm">
              <q-btn color="primary" label="Apply & Preview" type="button" @click="applyAndPreview()" />
    ...
    <q-dialog v-model="preview" maximized :persistent="false" full-height>
      <q-card>
        <q-card-section style="height: 100vh" class="q-pa-none">
          <div class="row justify-end q-pa-sm">
            <q-btn class="text-right" label="close" flat icon="close" v-close-popup />
          </div>
        </q-card-section>
      </q-card>
    </q-dialog>
...
const dataModel = ref<Dashboard>({
  ...props.meta.model,
  config: {
    companyFilter: { show: false, codes: [] },
  },
})
...
const getData = async (id: string | number) => {
  loading.value = true
  try {
    const res: any = await $api.get(`${props.meta.module}/${id}`)
    loading.value = false
...
const save = async () => {
  let statusapi = 600
  const res: any = await $api.post(props.meta.module, dataModel.value)
...
const update = async () => {
  let statusapi = 600
  const res: any = await $api.put(`${props.meta.module}/${dataModel.value.id}`, dataModel.value)
  if (res) statusapi = 200
  return statusapi
}
